...
//        Trie miss = new Trie();
//        // Separate ArrayList for misspelled words b/c it is hard to convert Trie to an array of words (count as you go)
...
//        return misspelled.toArray(new String[0]);
        ...
        TST dict = new TST();
        // Insert each word in the dictionary into the TST version
        for (String word : dictionary) dict.insert(word);
        // Create a TST for the misspelled words
        TST miss = new TST();
        // Separate ArrayList for misspelled words b/c it is hard to convert TST to an array of words (count as you go)
        ArrayList<String> misspelled = new ArrayList<>();
        for (String word : text) {
            // Add word to both the TST and ArrayList if it isn't in dictionary and isn't already in the miss TST
            if (!dict.lookup(word) && !miss.lookup(word)) {
                miss.insert(word);
                misspelled.add(word);
            }
